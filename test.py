...
def get_parser(**parser_kwargs):
    ...
    parser.add_argument('--ckpt_path', type=Path, required=True, help='Path to checkpoint file')
    return parser
...
class DataModuleFromConfig(LightningDataModule):
    ...
    def _test_dataloader(self):
        ...
        return DataLoader(self.datasets['test'], batch_size=self.batch_size,
                          num_workers=self.num_workers, worker_init_fn=init_fn, shuffle=False, persistent_workers=PERSISTENT_WORKERS)
...
def main():
    ...
    strategy = 'deepspeed_stage_2' # 'deepspeed_stage_3_offload' # deepspeed_stage_3_offload
    # strategy = 'ddp' # 'deepspeed_stage_3_offload' # deepspeed_stage_3_offload
    ...
    "params": {
        ...
        "wrap": False,
        "test": {
            "target": "ldm.data.tote_image_pair_v2.TotePairDataset",
            "params": {
                "state": "test",
                "dataset_dir": "dataset/shv1_saus_consolidate_image_pair",
                "buckets": [0, 1],
    ...
    model = load_model_from_config(model, args.ckpt_path, verbose=True)

    # trainer.test(model, data.test_dataloader, ckpt_path=args.ckpt_path, verbose=True)
    # trainer.test(model, data.test_dataloader, verbose=True)
    trainer.test(model, data, verbose=True)


def load_model_from_config(model, ckpt, verbose=True):
    print(f"Loading model from {ckpt}")
    pl_sd = torch_load(ckpt, map_location="cpu", weights_only=False)
